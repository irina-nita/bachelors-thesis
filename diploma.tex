...
As microcontrollers (MCUs) are increasingly becoming more advanced, embedded software developers have the ability to leverage the hardware for building real-time systems that can run simultaniously multiple applications. This often implies running a memory-efficient operating system with the minimum requirement of scheduling critical applications. Most of these systems are written in low-level languages, usually C/C++, due to good performance and low memory footprint, but are prone to memory-related security vulnerabilities[1].\
Tock is an operating system written in Rust, a low-level systems programming language similar to C. Unlike C, Rust offers memory safety features and implements some aspectes of the object-oriented paradigm.
