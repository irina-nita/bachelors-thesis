...
low memory footprint, most systems are written in low-level languages, usually C/C++, but are prone to memory-related security vulnerabilities[1].
...
time[2] and implements some aspects of the object-oriented paradigm[3].
...
The abstraction acquired through Rust generics and traits (define here trait) ensures portability, allowing the driver implementations to remain agnostic regarding the hardware platform they are running on.
